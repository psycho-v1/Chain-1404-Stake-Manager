...
(() => {
  ...
  function buildReport(pack, extra) {
    ...
    lines.push("--------------------------------------------------------------------------------");
    lines.push("This is the history served by GK Side (official GK Side (official rpc.bdagscan.com family)). Community");
    ...
    lines.push("GK Side, the wallet RPC must be pointed at that family first.");
    ...
    lines.push(line("  Subject blocked (GK Side)", div && div.ok ? (div.blocked ? "YES" : "no") : "unavailable"));
    lines.push("");
    ...
    if (div && div.ok && divPos > 0n) {
      lines.push("Verified.  The GK Side RPC family also returns a staking position for this");
      ...
      lines.push("Verified.  The GK Side RPC family answered and returned no position for");
      lines.push("this wallet and pool.");
    } else {
      lines.push("Limitation.  The GK Side RPC family did not answer. Community papers have");
    ...
    lines.push("  5. If a position exists only on the GK Side book: changing the wallet RPC");
