(() => {
  ...
  function drawCharts() {
    const mix = [
      { v: asNum(state.pos.wallet), c: "#f5c16c" },
      { v: asNum(state.pos.principal), c: "#7ee0b0" },
      { v: asNum(state.pos.yieldAmt), c: "#4ade80" },
      { v: asNum(state.pos.unbonding), c: "#fb7185" }
    ];
    pie($("chartMix"), mix);
    ...
    pie($("chartSplit"), [
      { v: state.minerSplit, c: "#f5c16c" },
      { v: state.stakerSplit, c: "#7ee0b0" }
    ]);
    const days = {};
    loadLog().forEach((row) => {
        ...
        <nav class="topbar px-3 py-2">
          <div class="container-xl d-flex align-items-center justify-content-between gap-2">
            ...
            <div class="d-flex align-items-center gap-2">
              <span id="status" class="pill wait">Checking…</span>
              <button type="button" class="btn btn-sm btn-ghost" id="btnTheme"><i class="fa-solid fa-sun"></i></button>
              <div class="wallet-cluster">
                <span id="walletSignal" class="sig sig-off"></span>
                <span id="rpcWarn" class="sig-warn d-none"><i class="fa-solid fa-triangle-exclamation"></i></span>
