(() => {
  ...
  function humanRevert(msg) {
    const t = String(msg || "");
    const rules = [
      [/EnforcedPause|paused/i, "Staking is paused on-chain. Do not retry."],
      [/NothingToClaim/i, "Nothing is claimable yet. The 4-week gate or the unbond clock is still running."],
      [/InsufficientUnlockedStake/i, "Part of this stake is still locked. The contract refused the unstake amount."],
      [/InsufficientLiquidStake/i, "Not enough unlocked stake on this pool for that amount."],
      [/InvalidPool/i, "That address is not a valid mining pool for this contract."],
      [/BelowMinStake/i, "Amount is below the 0.001 BDAG minimum."],
      [/CannotUnstakeZero/i, "Unstake amount cannot be zero."],
      [/NotAuthorized/i, "This wallet is not allowed to move that position."],
      [/TooManyRequests/i, "Too many open unbonding requests. Claim or wait before opening another."],
      [/AmountMismatch/i, "The amount in the call does not match the coins sent."],
      [/TransferFailed/i, "The contract could not send BDAG back."],
      [/wrong network|chain id/i, "Wallet is not on chain 1404."]
    ];
    for (const [re, text] of rules) if (re.test(t)) return text;
    return sanitize(t) || "The contract refused this send.";
  }
  function claimableNow() {
    const g = gates();
    const waiting = g.waitingUnbond.length > 0;
    const gateOpen = !g.firstUnlock || nowSec() >= g.firstUnlock;
    const yieldOpen = state.pos.yieldAmt > 0n && gateOpen && !waiting;
    const unbondReady = g.readyUnbond.reduce((s, r) => s + (r.amount || 0n), 0n);
    return {
      yieldAmt: yieldOpen ? state.pos.yieldAmt : 0n,
      unbond: unbondReady,
      total: (yieldOpen ? state.pos.yieldAmt : 0n) + unbondReady,
      waiting,
      gateOpen
    };
  }
  ...
  function lockedContract() {
    try { return JSON.parse(localStorage.getItem(K.picked) || sessionStorage.getItem(K.picked) || "null"); } catch { return null; }
  }
  ...
  function lockContract(row) {
    ...
    const picked = {
      ...
      label: row.label || row.name || short(row.address),
      name: row.name || "",
      at: Date.now()
    };
    localStorage.setItem(K.picked, JSON.stringify(picked));
    sessionStorage.setItem(K.picked, JSON.stringify(picked));
  ...
  function unlockContract() {
    ...
    localStorage.removeItem(K.picked);
    refresh();
  }
  ...
  function isGkHost(urlOrHost) {
    const raw = String(urlOrHost || "").toLowerCase();
    let host = raw;
    try { host = new URL(raw).hostname.replace(/^www\./, ""); } catch {
      host = raw.replace(/^https?:\/\//, "").split("/")[0].replace(/^www\./, "");
    }
    return (P.fork.rejectHosts || []).some((h) => host === h || host.endsWith("." + h) || raw.includes(h));
  }
  function denied(url) {
    try {
      const host = new URL(url).hostname.replace(/^www\./, "").toLowerCase();
      return isGkHost(host);
    } catch { return true; }
  }
  ...
  function toast(msg, kind) {
    ...
    if (!el) {
      ...
      el.id = "c1404toast";
      el.className = "c1404-toast";
      document.body.appendChild(el);
    }
    el.textContent = msg;
    el.className = "c1404-toast show" + (kind === "risk" ? " risk" : "");
    clearTimeout(window.__c1404toast);
    window.__c1404toast = setTimeout(() => el.classList.remove("show"), 2800);
  }
  function askConfirm(title, lines) {
    return new Promise((resolve) => {
      ...
      if ($("confirmBody")) {
        $("confirmBody").innerHTML = (lines || []).map((l) => "<li>" + l + "</li>").join("");
      }
      const go = $("btnConfirmGo");
      const cancel = $("btnConfirmNo");
      const finish = (v) => {
        go?.removeEventListener("click", onOk);
      ...
      go?.addEventListener("click", onOk);
      cancel?.addEventListener("click", onNo);
      if (window.bootstrap && el) window.bootstrap.Modal.getOrCreateInstance(el).show();
      else finish(confirm(title + "\n\n" + (lines || []).join("\n")));
    });
  }
  function showGkAbort(why) {
    toast("High Risk detected — GK Side send aborted", "risk");
    const existing = $("gkRiskModal");
    if (existing && window.bootstrap) {
      $("gkRiskBody").textContent = why || "This wallet RPC is on the GK Side book (official rpc.bdagscan.com family). Stake, unstake and claim from this UI are community-book only. GK Side figures stay view-only.";
      window.bootstrap.Modal.getOrCreateInstance(existing).show();
      return;
    }
    alert("High Risk detected.\n\n" + (why || "GK Side RPC is view-only. Send aborted."));
  }
  async function detectGkWallet() {
    if (!state.provider) return { gk: false, why: "" };
    try {
      const blk = await state.provider.request({
        method: "eth_getBlockByNumber",
        params: ["0x" + P.fork.firstDivergent.toString(16), false]
      });
      const h = String((blk && blk.hash) || "").toLowerCase();
      if (h && P.fork.gk316002 && h === P.fork.gk316002.toLowerCase()) {
        return { gk: true, why: "Wallet RPC matches the GK Side book (official rpc.bdagscan.com family). Sends from this UI are blocked." };
      }
      if (h && h !== String(P.fork.community316002).toLowerCase()) {
        return { gk: true, why: "Wallet RPC block 316002 hash is not the community pin. Treated as GK Side. Send aborted." };
      }
    } catch (e) {
      /* provider may not answer historical block; fall through */
    }
    return { gk: false, why: "" };
  }
  ...
  async function waitProviders(ms) {
    const t0 = Date.now();
  ...
  async function silentReconnect() {
    ...
    const remembered = localStorage.getItem(K.account);
    if (remembered && isAddr(remembered) && !state.account) {
      state.account = remembered;
      paintWalletBtn();
    }
    await waitProviders(1600);
    ...
    const entry =
      ...
      list[0];
    if (!entry) return !!state.account;
    let accs = [];
    try { accs = await entry.provider.request({ method: "eth_accounts" }); } catch {}
    if (!accs || !accs.length) return !!state.account;
    ...
    if (!sessionStorage.getItem("c1404.greeted")) {
      sessionStorage.setItem("c1404.greeted", "1");
      toast("Wallet connected");
    }
    return true;
  }
  ...
  function gates() {
    ...
    const canClaim = base && (
      ...
      (state.pos.yieldAmt > 0n && (!firstUnlock || now >= firstUnlock) && waitingUnbond.length === 0)
    );
  ...
  function claimWhy(g) {
    ...
    if (g.waitingUnbond.length) return "Claim paused while unbonding. Wait the 3-week clock, then claim.";
    if (state.pos.yieldAmt > 0n && g.firstUnlock && nowSec() < g.firstUnlock) {
      return "First-claim wait still running until " + dateFromSec(g.firstUnlock);
    }
  ...
  function paintActions() {
    ...
    if (line) {
      line.textContent = g.locked
        ? ("Using " + (g.locked.name || g.locked.label || short(g.locked.address)) + " · " + g.locked.address)
        : "All detected pools are listed. The most likely pool is marked. Tap Use this position to act — nothing is pre-selected.";
    }
  }
  ...
  function paintWait() {
    ...
    $("waitHint").textContent = w.amount
      ? (fmt(w.amount) + " BDAG can move after " + dateFromSec(w.end) + " · " + fmtRemain(left) + " left.")
      : ("Claim stays off until " + dateFromSec(w.end) + ". The contract can still refuse.");
    if (fill) {
  ...
  function paintFacts() {
    ...
    const head = state.headBlock ? Number(state.headBlock).toLocaleString() : "—";
    if ($("statusStrip")) {
      $("statusStrip").textContent =
        "Reading " + (state.rpcName || "community RPC") +
        " · chain 1404 · contract " + short(proxy) +
        " · block " + head +
        (pool ? (" · pool " + short(pool)) : " · no pool selected") +
        ". If the wallet RPC differs, the numbers will not match. GK Side is view-only.";
    }
    const why = (id, zero, ok) => { if ($(id)) $(id).textContent = zero || ok; };
    why("whyBal",
      state.pos.wallet === 0n ? "0 because this wallet has no liquid BDAG on this book." : null,
      "Liquid BDAG in the wallet. This is not staked.");
    why("whyStaked",
      state.pos.principal === 0n ? "0 because no BDAG is locked in the selected pool on this book." : null,
      "Locked in the staking contract. Not spendable until unstake + 3 weeks + claim.");
    why("whyRew",
      state.pos.yieldAmt === 0n ? "0 because the contract has not credited yield yet, the pool is wrong, or the stake is a tiny share of the pot." : null,
      "On-chain yield sitting in the contract. It does not arrive in the wallet by itself.");
    why("whyClaimable",
      c.total === 0n
        ? (c.waiting
          ? "0 because an unstake is in cooldown. Claims are paused."
          : (!c.gateOpen && state.pos.first
            ? ("0 until " + dateFromSec(g.firstUnlock) + ".")
            : "0 because nothing is unlocked to claim."))
        : null,
      "Ready to claim on this book now.");
    why("whyUnbond",
      state.pos.unbonding === 0n ? "0 because there is no open unstake request." : null,
      "Exit queue. Coins stay in the contract until the 3-week clock ends.");
    if ($("firstWhen")) {
      $("firstWhen").textContent = state.pos.first ? dateFromSec(Number(state.pos.first)) : "None";
    }
    why("whyFirst",
      !state.pos.first ? "No first deposit on this wallet yet." : null,
      g.firstUnlock && nowSec() < g.firstUnlock
        ? ("First claim opens " + dateFromSec(g.firstUnlock) + ".")
        : "First-claim wait has passed. The contract still decides each claim.");
    paintLiveNum("claimable", c.total);

    const ban = $("stateBanner");
    if (ban) {
      let html = "";
      let cls = "alert alert-warning mb-3";
      if (!state.account) {
        html = "Wallet is not connected. Numbers are public reads only. Connect once in the header.";
      } else if (state.blocked) {
        html = "This wallet is marked blocked on-chain. The UI cannot override that.";
        cls = "alert alert-danger mb-3";
      } else if (state.paused) {
        html = "Staking is paused on-chain. Do not send.";
        cls = "alert alert-danger mb-3";
      } else if (state.migrate) {
        html = "Contract implementation or owner changed. Review Contracts before any send.";
      } else if (state.rpcBad) {
        html = "Community RPC is not ready. Do not send until the header shows a live network.";
      } else if (state.pos.wallet > 0n && state.pos.principal === 0n && state.pos.unbonding === 0n) {
        html = "This wallet holds BDAG but has no staking position on the selected pool. Mining blocks here does not stake coins.";
      } else if (c.waiting) {
        const end = Math.min(...g.waitingUnbond.map((r) => Number(r.releaseTime)));
        html = "Unstake is a 3-week request, not a same-day send. Claims are paused until " + dateFromSec(end) + ". Adding more BDAG during this wait can restart the clock.";
      } else if (state.pos.yieldAmt > 0n && !c.gateOpen) {
        html = "Yield is accruing in the contract. You cannot claim until " + dateFromSec(g.firstUnlock) + ". Claiming later does not unstake principal.";
      } else if (c.total > 0n) {
        html = "Something is claimable now. Claim does not unstake remaining principal.";
        cls = "alert alert-success mb-3";
      } else if (state.pos.principal > 0n) {
        html = "Stake is active. Rewards may accrue in the contract. They do not appear in the wallet until you claim.";
        cls = "alert alert-secondary mb-3";
      }
      if (state.walletMiner && state.pos.principal === 0n) {
        html = (html ? html + " " : "") + "This address has produced blocks and still has 0 stake. Mining rewards are not staking yield.";
      }
      if (html) {
        ban.className = cls;
        ban.textContent = html;
      } else {
        ban.className = "alert d-none mb-3";
        ban.textContent = "";
      }
    }
  }
  ...
  function poolsFromUnstakeInput(input) {
    const raw = String(input || "").replace(/^0x/, "").toLowerCase();
    if (raw.slice(0, 8) !== "867f1a3e") return [];
    if (raw.length < 8 + 256) return [];
    const off = Number(BigInt("0x" + raw.slice(8 + 128, 8 + 192)));
    if (!Number.isFinite(off)) return [];
    const start = 8 + off * 2;
    if (start + 64 > raw.length) return [];
    const len = Number(BigInt("0x" + raw.slice(start, start + 64)));
    const out = [];
    let p = start + 64;
    for (let i = 0; i < len && i < 32; i++) {
      if (p + 64 > raw.length) break;
      out.push("0x" + raw.slice(p + 24, p + 64));
      p += 64;
    }
    return out.filter(isAddr);
  }
  function poolFromLog(it) {
    const topics = it.topics || [];
    const data = String(it.data || "").replace(/^0x/, "");
    const t0 = String(topics[0] || "").toLowerCase();
    if (P.sel.stakedTopic && t0 === String(P.sel.stakedTopic).toLowerCase()) {
      if (topics[2] && topics[2].length >= 42) return "0x" + String(topics[2]).slice(-40);
      if (data.length >= 128) return "0x" + data.slice(88, 128);
    }
    if (data.length >= 128 && /[0-9a-f]{40}$/.test(data.slice(88, 128))) {
      const maybe = "0x" + data.slice(88, 128);
      if (isAddr(maybe) && !isStakingProxy(maybe)) return maybe;
    }
    return "";
  }
  async function seedFromUnbonding(acc) {
    const pools = [];
    try {
      const hex = await ethCall(P.sel.getUnbondingRequests + addrWord(acc));
      const rows = decodeUnbonding(hex);
      state.unbond = rows;
      rows.forEach((r) => { if (r.pool) pools.push(r.pool); });
    } catch { }
    return pools;
  }
  async function seedFromLogs(acc) {
    const pools = [];
    const proxy = (activeContract().proxy || "").toLowerCase();
    let page = await explorerGet("/api/v2/addresses/" + acc + "/logs");
    let pages = 0;
    while (page && pages < 8) {
      pages += 1;
      for (const it of (page.items || [])) {
        const addr = String((it.address && it.address.hash) || it.address || "").toLowerCase();
        if (addr && addr !== proxy) continue;
        const pool = poolFromLog(it);
        if (pool) pools.push(pool);
      }
      const n = page.next_page_params;
      if (!n) break;
      const q = Object.entries(n).map(([k, v]) => encodeURIComponent(k) + "=" + encodeURIComponent(v)).join("&");
      page = await explorerGet("/api/v2/addresses/" + acc + "/logs?" + q);
    }
    return pools;
  }
  async function discoverOnChainPositions() {
    ...
    const add = (addr, label) => {
      if (!isAddr(addr) || isStakingProxy(addr)) return;
      const k = addr.toLowerCase();
      const hit = found.find((r) => r.address === k);
      if (hit) {
        if (label && !hit.sources.includes(label)) hit.sources.push(label);
        return;
      }
      found.push({ address: k, label: label || ("Pool " + short(k)), sources: [label || "Detected"] });
    };
    (await seedFromUnbonding(acc)).forEach((p) => add(p, "Unbonding request"));
    (await seedFromLogs(acc)).forEach((p) => add(p, "Staked log"));
    ...
    while (page && pages < 20) {
      ...
      for (const it of (page.items || [])) {
        ...
        const stakePool = poolFromStakeInput(input);
        if (stakePool) add(stakePool, "On-chain stake()");
        poolsFromUnstakeInput(input).forEach((p) => add(p, "On-chain unstake()"));
      }
      const n = page.next_page_params;
      if (!n) break;
      const q = Object.entries(n).map(([k, v]) => encodeURIComponent(k) + "=" + encodeURIComponent(v)).join("&");
    ...
    const picked = lockedContract();
    if (picked) add(picked.address, picked.label || "Selected");
  ...
  async function loadPositions(force) {
    ...
    for (const seed of seeds) {
      ...
      let name = seed.name || "";
      try {
        const probe = await probeMiner(seed.address);
        if (probe && probe.name) name = probe.name;
        seed.mining = probe && probe.mining;
        seed.validations = probe && probe.validations;
      } catch {}
      const row = { ...seed, ...inspected, isProxy: false, name };
      if (row.engaged) rememberPosition(row.address, name || row.label);
      rows.push(row);
    }
    rows.sort((a, b) => {
      if (a.engaged !== b.engaged) return a.engaged ? -1 : 1;
      const as = (a.principal || 0n) + (a.yieldAmt || 0n) + (a.unbonding || 0n);
      const bs = (b.principal || 0n) + (b.yieldAmt || 0n) + (b.unbonding || 0n);
      if (bs !== as) return bs > as ? 1 : -1;
      return b.principal > a.principal ? 1 : -1;
    });
    if (rows.length) {
      const live = rows.filter((r) => r.engaged);
      const likely = (live[0] || rows[0]);
      rows.forEach((r) => { r.likely = likely && r.address === likely.address; });
    }
    state.positions = rows;
    paintHomePositions(rows);
    return rows;
  }
  function renderPositionCards(rows, extra) {
    ...
    const card = (r) => {
      ...
      const title = r.name || r.label || (r.engaged ? "Open staking position" : "Looked-up pool");
      const sources = (r.sources || []).filter(Boolean).join(" · ");
      const badges = [
        r.likely ? '<span class="badge text-bg-warning">Most likely pool</span>' : "",
        r.engaged ? '<span class="badge text-bg-success">Engaged</span>' : '<span class="badge text-bg-secondary">No live balance</span>',
        isPicked ? '<span class="badge text-bg-success">Using this position</span>' : "",
        r.mining ? '<span class="badge text-bg-secondary">Miner</span>' : ""
      ].filter(Boolean).join(" ");
      return `<div class="glass pos-card p-3 mb-2 ${r.likely ? "likely" : ""} ${isPicked ? "using" : ""}">
        <div class="d-flex justify-content-between gap-2 flex-wrap">
          <div>
            <div class="fw-semibold">${title}</div>
            <div class="addr small">${r.address}</div>
            <div class="small mt-1">${badges}</div>
            ...
            ${sources ? `<div class="small text-secondary mt-1">Detected via ${sources}</div>` : ""}
          </div>
    ...
    const head = state.account
      ? (engaged.length
          ? `<p class="small mb-2">Your pools · ${engaged.length} live · total staked ${fmt(sumP, 6)} BDAG · total yield ${fmt(sumY, 6)} BDAG. The highlighted row is the most likely pool. Tap <strong>Use this position</strong> before any send — nothing is pre-selected.</p>`
          : `<p class="small mb-2">No live staking-contract position found yet. Search the mining-pool 0x you already used, not the staking contract.</p>`)
      : `<p class="small mb-2">Connect a wallet once in the header to list pools. The session stays until Disconnect.</p>`;
    box.innerHTML =
      head +
    ...
    paintHomePositions(state.positions || list);
  }
  function paintHomePositions(rows) {
    const box = $("homePosList");
    if (!box) return;
    const list = rows || state.positions || [];
    if (!state.account) {
      box.innerHTML = "<p class='small text-secondary mb-0'>Connect once to load positions. Session follows you across pages.</p>";
      return;
    }
    if (!list.length) {
      box.innerHTML = "<p class='small text-secondary mb-0'>No pool detected yet. Open Actions and search the mining-pool 0x from your original stake.</p>";
      return;
    }
    box.innerHTML = list.map((r) => {
      const title = r.name || r.label || short(r.address);
      return `<div class="coin-row">
        <div class="coin-ico letter">${(title || "P").slice(0, 1).toUpperCase()}</div>
        <div class="flex-grow-1">
          <div class="fw-semibold">${title} ${r.likely ? '<span class="badge text-bg-warning">Most likely</span>' : ""} ${r.engaged ? '<span class="badge text-bg-success">Engaged</span>' : ""}</div>
          <div class="addr small">${r.address}</div>
          <div class="small text-secondary">Staked ${fmt(r.principal, 4)} · Yield ${fmt(r.yieldAmt, 4)} · Unbonding ${fmt(r.unbonding, 4)}</div>
        </div>
      </div>`;
    }).join("");
  }
  ...
  async function refresh() {
    try {
      ...
      try {
        ...
        state.headBlock = Number(hexToBig(bn));
      } catch { }
      ...
      paintRewardTrack();
      paintFacts();
      if (state.account && state.walletMiner == null) {
        probeMiner(state.account).then((p) => {
          state.walletMiner = !!(p && p.mining);
          paintFacts();
        }).catch(() => {});
      }
      if ($("firstStake")) {
        if (!state.account) $("firstStake").textContent = "";
          ...
          $("firstStake").textContent = nowSec() >= unlock
            ? ("First deposit " + dateFromSec(Number(state.pos.first)) + " · first-claim wait has passed. The contract still decides each claim.")
            : ("First deposit " + dateFromSec(Number(state.pos.first)) + " · first claim opens " + dateFromSec(unlock) + ".");
        }
      }
      if ($("unbondList")) {
        ...
        $("unbondList").innerHTML = open.length
          ? open.map((r) => {
            ...
            return `<li>${fmt(r.amount)} BDAG · ${left ? ("until " + dateFromSec(Number(r.releaseTime)) + " · " + fmtRemain(left)) : "ready to claim"} · pool ${r.pool}</li>`;
          }).join("")
  ...
  async function sendTx(action, data, value, amountLabel) {
    ...
    const gk = await detectGkWallet();
    if (gk.gk) { showGkAbort(gk.why); return; }
    const g = gates();
  ...
  async function doStake() {
    ...
    } catch (e) { return alert("This stake would fail: " + humanRevert(e.message)); }
    const unlock = state.pos.first
      ? Number(state.pos.first) + P.constants.bondingSeconds
      : nowSec() + P.constants.bondingSeconds;
    const ok = await askConfirm(
      "Confirm stake",
      [
        fmt(amt) + " BDAG will leave the wallet.",
        "Destination contract: " + activeContract().proxy,
        "Those coins lock. They are not spendable.",
        "First claim opens around " + dateFromSec(unlock) + ".",
        "Any APY elsewhere is not a promise. Weekly yield is your share of the pool.",
        "Unstake is not same-day. Adding more during an open unstake can restart the wait."
      ]
    );
    if (!ok) return;
  ...
  async function doUnstake() {
    ...
    setPool(pool);
    let amt;
    const amtEl = $("amountUnstake") || $("amount");
    try { amt = parseAmount(amtEl && amtEl.value); } catch (e) { return alert(e.message); }
    const data = encodeUnstake(state.account, amt, [pool]);
    if (ledger === "divergent" || ledger === "gk") {
      showGkAbort("Unstake on GK Side is blocked. GK Side data is view-only.");
      return;
    }
    const g = gates();
    ...
    catch (e) { return alert("This unstake would fail: " + humanRevert(e.message)); }
    const release = nowSec() + P.constants.unbondingSeconds;
    const ok = await askConfirm(
      "Confirm 3-week unstake request",
      [
        "This is a request, not a withdrawal.",
        fmt(amt) + " BDAG stays in " + activeContract().proxy + " for about 3 weeks.",
        "Earliest move date around " + dateFromSec(release) + ".",
        "Claims pause during cooldown.",
        "Adding more BDAG during this wait can restart the clock.",
        "After the clock, tap Claim. Coins will not appear in the wallet before that."
      ]
    );
    if (!ok) return;
  ...
  async function doClaim() {
    ...
    if (ledger === "divergent" || ledger === "gk") {
      showGkAbort("Claim on GK Side is blocked. GK Side data is view-only.");
      return;
    }
    const g = gates();
    if (!g.canClaim) return alert(claimWhy(g));
    try { await rpc("eth_call", [{ to: activeContract().proxy, from: state.account, data }, "latest"]); }
    catch (e) { return alert("Claim is not allowed yet: " + humanRevert(e.message)); }
    const c = claimableNow();
    const ok = await askConfirm(
      "Confirm claim",
      [
        "Claim pays yield or matured unbonding. It does not unstake remaining principal.",
        "Accrued yield on-chain: " + fmt(state.pos.yieldAmt) + " BDAG.",
        "Claimable now: " + fmt(c.total) + " BDAG.",
        "4-week gate open: " + (c.gateOpen ? "yes" : ("no — " + dateFromSec(g.firstUnlock))),
        c.waiting ? "An unbond is still running, so yield claim stays paused." : "No open cooldown blocking yield."
      ]
    );
    if (!ok) return;
  ...
  function paintSplitView() {
    ...
    paintSplitCard($("splitCom"), "After split · community", "live", pack.afterCommunity,
      "Canonical community book. Only book this UI will send to.");
    paintSplitCard($("splitDiv"), "After split · GK Side", "view only", pack.afterDivergent,
      "Official rpc.bdagscan.com family. View only. Stake / unstake / claim against GK Side is aborted.");
    ...
    if (el && com && com.ok) {
      ...
      el.innerHTML = `
        ...
        <div class="split-kv"><span>GK Side wallet + position (view only)</span><strong>${div && div.ok ? fmt(div.wallet + dPos) : "unavailable"}</strong></div>
        <p class="small text-secondary mb-0 mt-2">Do not add the two books. They are incompatible ledgers that reuse chain ID 1404. GK Side is official rpc.bdagscan.com — view only.</p>`;
    }
    const hint = $("ledgerHint");
    if (hint && div && div.ok) {
      const dPos = div.principal + div.yieldAmt + div.unbonding;
      hint.textContent = dPos > 0n
        ? ("GK Side shows " + fmt(dPos) + " BDAG in stake/yield/unbond. View only — this UI will not send there.")
        : "GK Side answered. No position for this wallet + pool on that book.";
    }
  }
  ...
  async function doDivergentUnstake() {
    showGkAbort("Unstake on GK Side is blocked. Official rpc.bdagscan.com family is view-only.");
  }
  async function doDivergentClaim() {
    showGkAbort("Claim on GK Side is blocked. Official rpc.bdagscan.com family is view-only.");
  }
    ...
    if (header && !header.dataset.ready) {
      ...
      header.innerHTML = `
        ...
        <nav class="dock" aria-label="Wallet bar">
          ...
          <a class="${page === "actions" || page === "stake" || page === "unstake" || page === "claim" ? "active" : ""}" href="actions.html"><i class="fa-solid fa-layer-group"></i>Actions</a>
          <a class="${page === "insights" ? "active" : ""}" href="insights.html"><i class="fa-solid fa-chart-pie"></i>Insights</a>
        ...
        <div class="sheet" id="moreSheet">
          <div class="sheet-card">
            ...
            <div class="sheet-grid">
              <a class="sheet-item" href="actions.html"><i class="fa-solid fa-layer-group"></i>Stake / Unstake</a>
          ...
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-2">
              ...
              <div class="modal-body">
                <ul class="small mb-3" id="confirmBody"></ul>
                <p class="small text-secondary mb-0">Read the wallet prompt after this. The site cannot take a signature back.</p>
              </div>
              <div class="modal-footer border-0">
                <button type="button" class="btn btn-ghost" id="btnConfirmNo" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-accent" id="btnConfirmGo">I understand — continue</button>
              </div>
            </div>
          </div>
        </div>
        <div class="modal fade" id="gkRiskModal" tabindex="-1">
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-2">
              <div class="modal-header border-0">
                <h5 class="modal-title text-danger"><i class="fa-solid fa-triangle-exclamation me-2"></i>High Risk detected</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body">
                <p id="gkRiskBody" class="mb-2">This wallet RPC is on the GK Side book (official rpc.bdagscan.com family). Stake, unstake and claim from this UI are community-book only. GK Side figures stay view-only.</p>
                <p class="small text-secondary mb-0">Point the wallet at a community RPC, then retry. Do not send against GK Side from this app.</p>
              </div>
              <div class="modal-footer border-0">
                <button type="button" class="btn btn-accent" data-bs-dismiss="modal">Understood</button>
              </div>
            </div>
          </div>
        </div>
...
`;
    ...
    $("btnMaxUnstake")?.addEventListener("click", () => {
      const el = $("amountUnstake") || $("amount");
      if (el) el.value = fmt(state.pos.principal, 6).replace(/,/g, "");
    });
    $("sendLedger")?.addEventListener("change", (ev) => {
      const v = ev.target.value;
      if (v === "divergent" || v === "gk") {
        ev.target.value = "community";
        state.sendLedger = "community";
        showGkAbort("GK Side sends are blocked. Official rpc.bdagscan.com family is view-only.");
        return;
      }
      state.sendLedger = v;
    });
  ...
  function openActionTab() {
    ...
    const map = { stake: "tab-stake", unstake: "tab-unstake", claim: "tab-claim", unbond: "tab-claim", unbonding: "tab-claim" };
    const id = map[hash];
    if (!id) return;
    const btn = $(id);
  ...
  async function boot() {
    ...
    if (sessionOn()) {
      const remembered = localStorage.getItem(K.account);
      if (remembered && isAddr(remembered)) {
        state.account = state.account || remembered;
        paintWalletBtn();
      }
    }
    ...
    startWaitClock();
    openActionTab();
    window.addEventListener("hashchange", openActionTab);
